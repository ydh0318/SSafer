...
def build_fix_retry_prompt(finding_input: str, error_message: str) -> str:
    return "\n".join(
        [
            ...
            "If patches are included, each patch must include patchId, findingId, operation, filePath, oldText, newText, expectedFileHash, and requiresApproval.",
            "patches[].operation must be replace.",
            "patches[].filePath must be a repository-relative path using forward slashes.",
            "patches[].requiresApproval must be true.",
            "patches[].expectedFileHash must start with sha256:.",
            "Do not include masked values in oldText or newText.",
