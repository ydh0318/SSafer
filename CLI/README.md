...
이 문서는 CLI 명령어를 실제로 실행하는 사용자를 위한 상세 매뉴얼입니다. 프로젝트 전체 개요와 가장 짧은 시작 흐름은 루트 [README.md](../README.md)를 참고하세요.

웹 대시보드: <https://ssafer.co.kr>

---

## 빠른 실행 흐름

처음 보는 사용자는 아래 순서만 따라 하면 로컬 프로젝트 스캔, 업로드, 수정안 적용까지 진행할 수 있습니다.
...
| 상황 | 실행 위치 | 명령 |
| --- | --- | --- |
| 로컬 프로젝트를 스캔하고 웹에서 보기 | 프로젝트 루트 | `ssafer run --upload` |
| 이미 만든 최근 로컬 스캔 결과만 업로드 | 프로젝트 루트 | `ssafer upload` |
| EC2 서버 런타임 상태 점검 후 웹에서 보기 | 서버 홈 디렉터리 권장 | `ssafer server --upload` |
| 이미 만든 최근 서버 점검 결과만 업로드 | 서버 홈 디렉터리 권장 | `ssafer upload --server` |
| 웹에서 Agent 버튼으로 스캔/수정 요청 처리 | 스캔할 PC 또는 서버 | `ssafer agent` |
| AI가 만든 수정안을 CLI에서 직접 적용 | 프로젝트 루트 | `ssafer apply` |
