...
- Jenkins URL: `http://k14b105.p.ssafy.io:9090` 또는 운영에서 노출한 Jenkins URL
...
```text
http://k14b105.p.ssafy.io:9090/project/<JENKINS_JOB_NAME>
```
...
```text
URL: http://k14b105.p.ssafy.io:9090/project/<JENKINS_JOB_NAME>
Secret token: Jenkins Job에서 생성한 token
Trigger: Push events
Branch filter: develop
SSL verification: Jenkins URL이 HTTP이면 비활성화, HTTPS이면 활성화
```
...
- `develop` 외 브랜치 자동 배포는 운영 안정성을 위해 비활성화합니다.
