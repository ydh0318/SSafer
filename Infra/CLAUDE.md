...
- EC2 #2: 초기화 절차는 아래 "EC2 #2 서버 초기화 절차" 섹션 참고. Jenkinsfile에서 `DEPLOY_EC2_2=false` 기본값으로 스킵 가능
...
EC2 #2 배포 활성화 전 아래 credential 추가 등록 필요:

| ID | Kind | 용도 |
|----|------|------|
| `ec2-2-host` | Secret text | EC2 #2 public IP 또는 DNS |
| `ec2-2-ssh-key` | SSH Username with private key | ubuntu / EC2 #2 SSH private key |
...
Jenkins가 EC2 #2에 SSH로 접속해 `deploy-ec2-2.sh`를 실행하는 구조다.
EC2 #2에 Jenkins는 설치하지 않는다.
...
# 터미널 재접속 후 docker ps로 확인
```

### 2. 배포 디렉토리 및 .env 설정

```bash
mkdir -p /home/ubuntu/ssafer/S14P31B105/Infra/docker/ec2-2/prod
mkdir -p /home/ubuntu/ssafer/S14P31B105/Infra/scripts

# Jenkins가 scp로 docker-compose.yml과 deploy-ec2-2.sh를 전송한다
# .env는 수동으로 생성 (절대 Git에 포함하면 안 됨)
...
EC2 #1의 Spring이 `ANALYSIS_SERVER_URL=http://<EC2_2_PRIVATE_IP>:8000`으로 FastAPI를 호출하므로 EC2 #1 private IP만 8000 허용.

### 4. EC2 #1 UFW 추가 (EC2 #2 private IP → 8080)

EC2 #2의 FastAPI가 Spring 콜백(`http://<EC2_1_PRIVATE_IP>:8080/...`)을 호출하므로 EC2 #1에서도 설정:
...
### 5. Jenkins Credential 등록

Jenkins 관리 → Credentials → System → Global credentials에 아래 두 항목 추가:

| Kind | ID | 값 |
|------|----|----|
| `Secret text` | `ec2-2-host` | EC2 #2 public IP (또는 DNS) |
| `SSH Username with private key` | `ec2-2-ssh-key` | ubuntu / EC2 #2 SSH private key |

### 6. Jenkins 파이프라인 활성화

Jenkins Job `ssafer-prod-deploy` → Build with Parameters → `DEPLOY_EC2_2=true` 체크 후 실행.
이후 push event에서도 자동으로 EC2 #2 배포가 포함된다.

첫 배포 전 EC2 #2에서 수동으로 구조 확인:

```bash
# EC2 #2에서
ls /home/ubuntu/ssafer/S14P31B105/Infra/docker/ec2-2/prod/
# docker-compose.yml, .env 모두 있어야 함
